...
const Book = (props)=>{
 ...
 // rowSelection object indicates the need for row selection
  ...
  useEffect(() => {
    setTimeout(() => {
        BooksDao.getbooks({page:page-1,size:size,cle:cle}).then(data=>{
          setBooks(data.docs)
  ...
  const onChange=async (pageNumber)=> {
        setLoading(true)
        const b = await BooksDao.getbooks({page:pageNumber-1,size:size,cle:cle})
        setTimeout(() => {
          setLoading(false)
          setPage(pageNumber)
          setBooks(b.docs)
    ...
    const searsh =async(motCle)=>{
      //searsh
        setLoading(true)
        const b = await BooksDao.getbooks({page:page-1,size:size,cle:motCle})
        setTimeout(() => {
          setLoading(false)
          setBooks(b.docs)
          setLength(b.length)
